...
func (w *MessageWriter) init(ws *WebSocket, opcode int) {
	...
	if ws.opts.compressAlloter != nil && ws.useFlateWrite == true {
		w.frame_flate.init(ws, opcode)
		w.flate_writer, _ = ws.opts.compressAlloter.NewWriter(&w.frame_flate)
	}
}
...
func (w *MessageWriter) reset(ws *WebSocket, opcode int) {
	...
	if w.flate_writer != nil && ws.opts.compressAlloter != nil {
		w.frame_flate.Reset(ws, opcode)
		ws.opts.compressAlloter.ResetWriter(w.flate_writer, &w.frame_flate)
	}
}
...
func (w *MessageWriter) close() error {
	...
	if w.flate_flag == true && w.flate_writer != nil {
		err := w.opts.compressAlloter.FlushWriter(w.flate_writer)
		if err != nil {
			return err
		}
